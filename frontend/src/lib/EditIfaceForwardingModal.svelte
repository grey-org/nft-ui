<script>
  ...
  import { editIfaceForwardingRule, pauseRefresh, resumeRefresh } from './stores.js';
  ...
  >
    <div class="flex items-center justify-between mb-4" style="border-bottom: 0.5px solid var(--border); padding-bottom: 10px;">
      <div class="flex items-center gap-2">
        ...
        <span style="font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: var(--text-muted);">Edit Interface Forwarding Rule</span>
      </div>
      <button
    ...
    <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
      ...
      <div style="font-size: 11px; padding: 8px 10px; border-radius: 2px; margin-bottom: 16px; background-color: var(--surface-hover); color: var(--text-muted); border: 0.5px solid var(--border);">
        note: editing will briefly remove and re-add the rule.
      </div>
